...
db.objectsData.updateMany( {}, { $rename: { "guid": "globalId" } } );
db.objectsData.find( filterNull ).forEach( function (x) { x.globalId = new UUID(); db.objectsData.save(x);});
db.objectsData.find( filter ).forEach( function (x) { x.globalId = new UUID(x.globalId); db.objectsData.save(x);});
db.objectsData.find( filterNull ).count();
